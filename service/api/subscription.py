...
async def create_subscription(
    ...
    new_subscription = Subscription(type=subscription.type, user_id=user.id, is_active=True)
...
async def get_subscription_info(subscription_id: int, db: Session = Depends(get_db), user_mail: str = Depends(get_user_from_cookie)):
...
async def get_subscription_list(request: Request, db: Session = Depends(get_db),user_mail: str = Depends(get_user_from_cookie)):
    user = db.query(User).filter(User.email ==user_mail).first()
    if not user:
        raise HTTPException(status_code=404, detail="Пользователь не найден")
...
async def update_subscription(
    subscription_id: int, subscription_data: SubscriptionCreate, db: Session = Depends(get_db),user_mail: str = Depends(get_user_from_cookie)
):
    subscription = db.query(Subscription).filter(Subscription.id == subscription_id).first()
    if not subscription:
        raise HTTPException(status_code=404, detail="Подписка не найдена")
...
async def cancel_subscription(subscription_id: int, db: Session = Depends(get_db),user_mail: str = Depends(get_user_from_cookie)):
    subscription = db.query(Subscription).filter(Subscription.id == subscription_id).first()
    if not subscription:
        raise HTTPException(status_code=404, detail='Подписка не найдена')
